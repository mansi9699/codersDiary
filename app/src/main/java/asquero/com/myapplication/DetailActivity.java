...
public class DetailActivity extends AppCompatActivity {
    ...
    private RecyclerView.Adapter DetailListAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!networkConnectivity()) {
            ...
            loadDetails(noInternetConnection);
        }
    ...
    public void loadDetails(TextView noInternetConnection) {
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        try {
                            ...
                            if (responseCounter == 3) {
                                ...
                                size = detailLists.size();

                                sort_list_live();

                                DetailListAdapter = new DetailListAdapter(detailLists, DetailActivity.this);
    ...
    public void sort_list_live(){
        ...
        for(int b = 0 ; b < size ; b++) {
            ...
            Log.i("listLiveInitial", detailLists.get(b).getStartDate()+"***"+b);

            for (int a = 0; a < size-1; a++) {

                String startDate1 = detailLists.get(a).getStartDate();
                int sd1 = dmy.returnDateNum(startDate1);
                Log.i("startdate1", startDate1+"***"+sd1);

                String startDate2 = detailLists.get(a + 1).getStartDate();
                int sd2 = dmy.returnDateNum(startDate2);
                Log.i("startyear2", startDate2+"***"+sd2);

                if (sd1 > sd2) {

                    ll = detailLists.get(a);
                    detailLists.set(a, detailLists.get(a+1));
                    detailLists.set(a+1, ll);
                }
            }
            //listLiveSort.add(ll);
            Log.i("DetailListFinal", detailLists.get(b).getStartDate()+"***"+b);
        }
